...
$.validator.addMethod("loginname", function(value) {
	if(value.length==0) {return true;}
	var p = /^(?:13\d|15[89])-?\d{5}(\d{3}|\*{3})$/ ;
	var p2 = /^(([0\+]\d{2,3}-)?(0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$/;
	var p3 = /^([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+@([a-zA-Z0-9]+[_|\_|\.]?)*[a-zA-Z0-9]+\.[a-zA-Z]{2,3}$/;
	return (p.exec(value) ? true : false) || (p2.exec(value) ? true : false) || (p3.exec(value) ? true : false);
}, "Please enter only letters and digits");
